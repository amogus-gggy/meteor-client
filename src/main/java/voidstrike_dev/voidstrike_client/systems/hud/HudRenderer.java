...
import voidstrike_dev.voidstrike_client.renderer.*;
import voidstrike_dev.voidstrike_client.renderer.*;
import voidstrike_dev.voidstrike_client.renderer.text.CustomTextRenderer;
import voidstrike_dev.voidstrike_client.renderer.text.Font;
import voidstrike_dev.voidstrike_client.renderer.text.VanillaTextRenderer;
...
public class HudRenderer {
    ...
    public double text(String text, double x, double y, Color color, boolean shadow, double scale) {
        ...
        if (fontHolder == null) {
            // Fallback to vanilla text renderer if custom font holder is not available
            VanillaTextRenderer.INSTANCE.scale = scale * 2;
            return VanillaTextRenderer.INSTANCE.render(text, x, y, color, shadow);
        }
        ...
        if (font == null) {
            // Fallback to vanilla text renderer if custom font is not available
            VanillaTextRenderer.INSTANCE.scale = scale * 2;
            return VanillaTextRenderer.INSTANCE.render(text, x, y, color, shadow);
        }
    ...
    public double textWidth(String text, boolean shadow, double scale) {
        ...
        if (hud.hasCustomFont()) {
            ...
            if (font != null) {
                double width = font.getWidth(text, text.length());
    ...
    public double textHeight(boolean shadow, double scale) {
        if (hud.hasCustomFont()) {
            Font font = getFont(scale);
            if (font != null) {
                double height = font.getHeight() + 1;
    ...
    private FontHolder getFontHolder(double scale, boolean render) {
        ...
        if (fontHolder == null) {
            ...
            if (fontHolder != null) {
                fontCache.put(height, fontHolder);
            }
        }
        return fontHolder;
    }
    ...
    private Font getFont(double scale) {
        FontHolder holder = getFontHolder(scale, false);
        return holder != null ? holder.font : null;
    }
    ...
    private static FontHolder loadFont(int height) {
        if (Fonts.RENDERER == null) {
            MeteorClient.LOG.error("Cannot load font: Fonts.RENDERER is null for height: {}", height);
            // Return a placeholder or handle gracefully
            return null;
        }
        ...
        try {
            ...
            MeteorClient.LOG.error("Failed to load font for HUD", e);
            return null;
        }
    }
